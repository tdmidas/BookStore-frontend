...
export const login = (email, password) => async (dispatch) => {
	try {
		...
		const { data } = await axios.post("http://localhost:8000/api/v1/auth/login", { email, password }, config);
...
export const listUsers = () => async (dispatch, getState) => {
	try {
		...
		const { data } = await axios.get("http://localhost:8000/api/v1/user/", config);
...
export const updateUser = (user) => async (dispatch, getState) => {
	try {
		...
		const { data } = await axios.put(`http://localhost:8000/api/v1/user/${user._id}`, user, config);
